export interface Card {
  ...
  value: number;
  suit: string;
  location: 'deck' | 'hand' | 'discard_up' | 'discard_down';
...
export interface GameState {
  ...
  phase: 'lobby' | 'dealing' | 'peeking' | 'playing' | 'round_end' | 'game_end' | 'terminated';
  max_players: number;
...
export function logAction(game: GameState, seat: number, message: string): void {
...
export function maskCard(c: Card, seat: number | null, phase: string): Card {
...
export function maskStateForPlayer(game: GameState, seat: number | null): GameState {
  return {
    ...game,
...
export function dealRound(game: GameState): void {
...
export function discardDrawnCard(game: GameState, seat: number, usedAbility: boolean): void {
...
export function refreshDeckIfEmpty(game: GameState): void {
...
export function endRound(game: GameState): void {
  game.cards.forEach(c => {
    if (c.location === 'hand') c.face_up = true;
  });
