...
const clients = new Map<WebSocket, { gameId: string; seatIndex: number; userId: string; username: string }>();
const turnTimers = new Map<string, NodeJS.Timeout>();
const idleTimers = new Map<string, NodeJS.Timeout>();
const rateLimit = new Map<WebSocket, number>();
...
async function verifyToken(token: string): Promise<{ id: string; email?: string; user_metadata?: any }> {
...
async function handleJoin(ws: WebSocket, gameId: string, token: string) {
  ...
  if (!p) {
    ...
    if (game.players.length >= game.max_players) throw new Error('Game full');
...
function handlePlayerTimeout(gameId: string, seatIndex: number) {
...
async function handleAction(ws: WebSocket, msg: any) {
  const client = clients.get(ws);
  if (!client) throw new Error('Not joined');
  const now = Date.now();
  const lastAction = rateLimit.get(ws) || 0;
  if (now - lastAction < 500) throw new Error('Rate limit exceeded');
  ...
  switch (msg.type) {
    case 'ready':
      ...
      if (p) p.is_ready = !p.is_ready;
      break;
    case 'start':
      if (client.seatIndex !== 0) throw new Error('Only host can start');
    ...
    case 'draw':
      ...
      if (game.cards.some(c => c.owner_seat === client.seatIndex && c.hand_index === 99)) throw new Error('Already drawn');
      refreshDeckIfEmpty(game);
      const deckIdx = game.cards.findIndex(c => c.location === 'deck');
      if (deckIdx === -1) throw new Error('No cards');
    ...
    case 'swapDrawn':
      ...
      if (!handCard || !drawn) throw new Error('Card not found');
      drawn.location = 'hand';
    ...
    case 'keepDiscard':
      ...
      if (!toDiscard) throw new Error('No drawn card');
    ...
    case 'peek':
      ...
      if (!peekDrawn || (peekDrawn.value !== 6 && peekDrawn.value !== 7)) throw new Error('Invalid ability usage');
      const peekCard = game.cards.find(c => c.owner_seat === client.seatIndex && c.hand_index === msg.cardIndex && c.location === 'hand');
      if (!peekCard) throw new Error('Card not found');
    ...
    case 'spy':
      ...
      if (!spyDrawn || (spyDrawn.value !== 8 && spyDrawn.value !== 9)) throw new Error('Invalid ability usage');
      if (game.cabo_called && game.cabo_caller_seat === msg.targetSeat) throw new Error('Locked');
      const spyCard = game.cards.find(c => c.owner_seat === msg.targetSeat && c.hand_index === msg.cardIndex && c.location === 'hand');
      if (!spyCard) throw new Error('Card not found');
      discardDrawnCard(game, client.seatIndex, true);
    ...
    case 'blindSwap':
      ...
      if (!bsDrawn || (bsDrawn.value !== 10 && bsDrawn.value !== 11)) throw new Error('Invalid ability usage');
      if (game.cabo_called && game.cabo_caller_seat === msg.targetSeat) throw new Error('Locked');
      ...
      if (!myCard || !targetCard) throw new Error('Card not found');
    ...
    case 'swapWithPeek':
      ...
      if (!swDrawn || swDrawn.value < 12) throw new Error('Invalid ability usage');
      if (game.cabo_called && game.cabo_caller_seat === msg.targetSeat) throw new Error('Locked');
      ...
      if (!sMyCard || !sTargetCard) throw new Error('Card not found');
      if (msg.phase === 'reveal') {
    ...
    case 'callCabo':
      ...
      if (game.cards.some(c => c.owner_seat === client.seatIndex && c.hand_index === 99)) throw new Error('Must call before draw');
      ...
      break;
    case 'nextRound':
      if (game.phase !== 'round_end') throw new Error('Round not over');
...
function verifyTurn(game: GameState, seat: number) {
  if (game.phase !== 'playing') throw new Error('Not in playing phase');
  if (game.current_turn_seat !== seat) throw new Error('Not your turn');
}

function startPeekingTimer(gameId: string) {
...
function resetIdleTimeout(gameId: string) {
...
function destroyRoom(gameId: string) {
  clearTurnTimer(gameId);
...
function broadcastToRoom(gameId: string, msg: any) {
